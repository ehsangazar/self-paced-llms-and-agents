...
Runnable companion for Session 2. Same idea as [S1](../01-s01-why-demos-die): a
...
```bash
npm run lab modules/01-workflows-and-agents/05-s02-code-model-boundary/workflow-vs-agent/as-workflow.ts
```
...
```bash
npm run lab modules/01-workflows-and-agents/05-s02-code-model-boundary/workflow-vs-agent/as-agent.ts
```
...
The week's hands-on lab is [Lab 1 · Build a Workflow Router](../10-lab-workflow-router).
